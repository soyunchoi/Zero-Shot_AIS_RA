...
if DATASET_NAME == 'd2sa':
    ...
    OUTPUT_DIR = "./outputs/D2SA_aug_amodal_visible_retrieval_250917"
...
if __name__ == '__main__':
    ...
    best_metric = 0.0  # 현재 단계에 따라 visible_mIoU 또는 amodal_mIoU 사용
    patience_counter = 0
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if is_best:
            ...
            patience_counter += 1
            print(f"현재 {metric_name}: {current_metric:.4f} (최고: {best_metric:.4f})")
        ...
        print(f"\n현재 최고 {metric_name}: {best_metric:.4f} (에포크 {checkpoint_manager.best_epoch})")
        
        # Early stopping은 amodal 단계에서만 적용 (visible 단계에서는 적용하지 않음)
        if training_stage != 'visible_only' and patience_counter >= PATIENCE:
            print(f"\n{PATIENCE} 에포크 동안 성능 개선이 없어 학습을 조기 종료합니다.")
            break

    print(f"\n=== 학습 완료 ===\n최종 최고 메트릭: {best_metric:.4f}")
