...
class GaussianDistributionRetrieval:
    """Gaussian Distribution 기반 Retrieval 클래스"""
    def __init__(self, retrieval_root="/root/datasets/UOAIS/UOAIS-Sim/retrieval", 
                 save_retrieval_results=True, output_dir="./outputs"):
        ...
        # Gaussian distribution 저장용
        self.gaussian_distributions = {}
        self.distribution_features = {}  # mean, cov를 벡터로 변환한 것들
        
        # FAISS 인덱스 (효율적인 검색을 위해)
        ...
        # Gaussian distribution 사전 계산
        self._precompute_gaussian_distributions()
        
        print(f"Gaussian Distribution Retrieval 데이터베이스 로드 완료: {len(self.gaussian_distributions)}개 이미지")
    ...
    def _precompute_gaussian_distributions(self):
        """RA 이미지들의 Gaussian distribution 사전 계산"""
        print("Gaussian distribution 사전 계산 시작...")
        
        # 처리할 이미지 수 제한 (메모리 효율성을 위해)
        max_images = min(1000, len(self.image_features))
        print(f"Distribution 계산 대상: {max_images}개 이미지")
        ...
        for img_name, info in list(self.image_features.items())[:max_images]:
            try:
                ...
                if processed_count % 100 == 0:
                    print(f"Distribution 계산 진행률: {processed_count}/{max_images}")
                ...
                # Gaussian distribution 계산
                gaussian_params = self._compute_gaussian_distribution(mask_cropped)
                if gaussian_params is not None:
                    self.gaussian_distributions[img_name] = gaussian_params
                    
                    # 검색을 위한 feature 벡터 생성 (mean + flattened cov)
                    mean, cov = gaussian_params
                    feature_vector = np.concatenate([mean.flatten(), cov.flatten()])
                    self.distribution_features[img_name] = feature_vector
                
            except Exception as e:
                print(f"이미지 {img_name} Gaussian distribution 계산 실패: {e}")
                continue
        
        print(f"Gaussian distribution 사전 계산 완료: {len(self.gaussian_distributions)}개 이미지")
        
        # FAISS 인덱스 구축
        self._build_faiss_index()
    
    def _build_faiss_index(self):
        """FAISS 인덱스 구축 (효율적인 검색을 위해)"""
        if not FAISS_AVAILABLE or not self.distribution_features:
            print("[WARNING] FAISS를 사용할 수 없거나 feature가 없어 인덱스 구축을 건너뜁니다.")
            return
        
        try:
            # Feature 벡터들을 numpy array로 변환
            feature_vectors = []
            image_names = []
            
            for img_name, feature_vector in self.distribution_features.items():
                feature_vectors.append(feature_vector)
                image_names.append(img_name)
            
            feature_matrix = np.array(feature_vectors).astype('float32')
            self.feature_dim = feature_matrix.shape[1]
            
            print(f"[INFO] FAISS 인덱스 구축 시작: {feature_matrix.shape[0]}개 벡터, 차원: {self.feature_dim}")
            ...
            # 이미지 이름 매핑 저장
            self.image_name_mapping = image_names
            
            print(f"[INFO] FAISS 인덱스 구축 완료: {self.faiss_index.ntotal}개 벡터")
            
        except Exception as e:
            print(f"[ERROR] FAISS 인덱스 구축 실패: {e}")
            self.faiss_index = None
    
    def _compute_gaussian_distribution(self, image):
        """이미지에서 Gaussian distribution (mean, covariance) 계산"""
        try:
            # PIL Image를 numpy array로 변환
            img_array = np.array(image)
            
            # RGB를 grayscale로 변환 (또는 각 채널별로 처리)
            if len(img_array.shape) == 3:
                # 각 픽셀을 3D 좌표로 변환 (R, G, B)
                h, w, c = img_array.shape
                pixels = img_array.reshape(-1, c)  # (N, 3)
                
                # 픽셀 위치 정보도 포함 (spatial + color information)
                y_coords, x_coords = np.meshgrid(np.arange(h), np.arange(w), indexing='ij')
                coords = np.stack([y_coords.flatten(), x_coords.flatten()], axis=1)  # (N, 2)
                
                # 위치와 색상을 결합 (normalize coordinates)
                coords_normalized = coords / np.array([h, w])  # [0, 1] 범위로 정규화
                features = np.concatenate([coords_normalized, pixels], axis=1)  # (N, 5)
            else:
                # Grayscale 이미지인 경우
                h, w = img_array.shape
                y_coords, x_coords = np.meshgrid(np.arange(h), np.arange(w), indexing='ij')
                coords = np.stack([y_coords.flatten(), x_coords.flatten()], axis=1)
                coords_normalized = coords / np.array([h, w])
                features = np.concatenate([coords_normalized, img_array.reshape(-1, 1)], axis=1)  # (N, 3)
            
            # Gaussian distribution 파라미터 계산
            mean = np.mean(features, axis=0)
            cov = np.cov(features.T)
            
            # Covariance matrix가 singular하지 않도록 regularization
            cov += np.eye(cov.shape[0]) * 1e-6
            
            return mean, cov
            
        except Exception as e:
            print(f"Gaussian distribution 계산 실패: {e}")
            return None
    
    def compute_distribution_similarity(self, query_image, query_bbox, top_k=5):
        """입력 이미지와 RA 이미지들의 Gaussian distribution 유사도 계산"""
        print(f"[DEBUG] compute_distribution_similarity 시작: top_k={top_k}")
        
        # Query 이미지의 bbox 영역에서 Gaussian distribution 계산
        query_distribution = self._compute_query_distribution(query_image, query_bbox)
        if query_distribution is None:
            print("[WARNING] Query distribution 계산 실패")
            return []
        
        query_mean, query_cov = query_distribution
        query_feature = np.concatenate([query_mean.flatten(), query_cov.flatten()])
        
        # FAISS 인덱스가 있으면 효율적인 검색 사용
        if self.faiss_index is not None:
            return self._compute_similarity_with_faiss(query_feature, query_mean, query_cov, top_k)
        else:
            return self._compute_similarity_brute_force(query_feature, query_mean, query_cov, top_k)
    
    def _compute_similarity_with_faiss(self, query_feature, query_mean, query_cov, top_k):
        """FAISS를 사용한 효율적인 유사도 계산"""
        try:
            # FAISS 검색 (L2 거리 기반)
            query_vector = query_feature.reshape(1, -1).astype('float32')
            ...
            for i, (dist, idx) in enumerate(zip(distances[0], indices[0])):
                ...
                img_name = self.image_name_mapping[idx]
                mean, cov = self.gaussian_distributions[img_name]
                
                # KL divergence 계산 (더 정확한 유사도)
                kl_div = self._compute_symmetric_kl_divergence(query_mean, query_cov, mean, cov)
                kl_similarity = np.exp(-kl_div)
                
                # L2 거리를 유사도로 변환
                l2_similarity = 1.0 / (1.0 + dist)
                
                # 두 유사도를 결합
                final_similarity = 0.6 * kl_similarity + 0.4 * l2_similarity
                
                info = self.image_features.get(img_name, {})
                similarities.append((img_name, final_similarity, info))
            
            # 유사도 기준으로 정렬하여 Top-K 반환
            similarities.sort(key=lambda x: x[1], reverse=True)
            top_results = similarities[:top_k]
            
            print(f"[DEBUG] Top-{top_k} 결과 (FAISS + Gaussian Distribution 기반):")
    ...
    def _compute_similarity_brute_force(self, query_feature, query_mean, query_cov, top_k):
        """Brute force 방식의 유사도 계산 (FAISS가 없을 때)"""
        similarities = []
        
        # 각 RA 이미지와의 유사도 계산
        for img_name, (mean, cov) in self.gaussian_distributions.items():
            try:
                # KL divergence 계산 (대칭화)
                kl_div = self._compute_symmetric_kl_divergence(
                    query_mean, query_cov, mean, cov
                )
                
                # Cosine similarity 계산 (feature vector 기반)
                stored_feature = self.distribution_features[img_name]
                cos_sim = cosine_similarity([query_feature], [stored_feature])[0][0]
                
                # 최종 유사도 점수 (KL divergence는 낮을수록 유사, cosine similarity는 높을수록 유사)
                # KL divergence를 유사도로 변환: exp(-kl_div)
                kl_similarity = np.exp(-kl_div)
                
                # 두 유사도를 결합 (가중 평균)
                final_similarity = 0.7 * kl_similarity + 0.3 * cos_sim
                ...
                print(f"[ERROR] Distribution 유사도 계산 중 오류: {e}")
                continue
        ...
        print(f"[DEBUG] Top-{top_k} 결과 (Brute Force + Gaussian Distribution 기반):")
        for i, (img_name, sim, info) in enumerate(top_results):
            print(f"  {i+1}. {img_name}: similarity={sim:.4f}, surface={info.get('surface', 'N/A')}")
        
        return top_results
    
    def _compute_query_distribution(self, query_image, query_bbox):
        """쿼리 이미지의 bbox 영역에서 Gaussian distribution 계산"""
        try:
            ...
            # 텐서를 PIL Image로 변환
            query_pil = self._tensor_to_pil(query_cropped[0])
            
            # Gaussian distribution 계산
            return self._compute_gaussian_distribution(query_pil)
            
        except Exception as e:
            print(f"[ERROR] Query distribution 계산 실패: {e}")
            return None
    
    def _compute_symmetric_kl_divergence(self, mean1, cov1, mean2, cov2):
        """대칭화된 KL divergence 계산"""
        try:
            # KL(P||Q) + KL(Q||P) / 2
            kl_12 = self._compute_kl_divergence(mean1, cov1, mean2, cov2)
            kl_21 = self._compute_kl_divergence(mean2, cov2, mean1, cov1)
            return (kl_12 + kl_21) / 2
        except:
            return float('inf')
    
    def _compute_kl_divergence(self, mean1, cov1, mean2, cov2):
        """KL divergence 계산"""
        try:
            # Multivariate Gaussian KL divergence
            # KL(P||Q) = 0.5 * (tr(Σ2^-1 Σ1) + (μ2-μ1)^T Σ2^-1 (μ2-μ1) - k + ln(|Σ2|/|Σ1|))
            
            # Covariance matrix 역행렬 계산
            ...
            return max(0, kl_div)  # KL divergence는 항상 >= 0
            
        except Exception as e:
            print(f"[ERROR] KL divergence 계산 실패: {e}")
            return float('inf')
    ...
    def get_retrieval_features(self, query_image, query_bbox, top_k=5):
        """쿼리 이미지에 대한 retrieval Gaussian distribution features 반환"""
        print(f"[DEBUG] get_retrieval_features 시작: top_k={top_k}")
        
        top_similar = self.compute_distribution_similarity(query_image, query_bbox, top_k)
        ...
        for i, (img_name, similarity, info) in enumerate(top_similar):
            try:
                ...
                # Gaussian distribution feature 가져오기
                if img_name in self.gaussian_distributions:
                    mean, cov = self.gaussian_distributions[img_name]
                    
                    # Distribution feature를 텐서로 변환
                    mean_tensor = torch.from_numpy(mean).float().to(device)
                    cov_tensor = torch.from_numpy(cov).float().to(device)
                    
                    print(f"[DEBUG] Distribution feature - mean shape: {mean_tensor.shape}, cov shape: {cov_tensor.shape}")
                    
                    retrieval_features.append({
                        'image_name': img_name,
                        'similarity': similarity,
                        'mean': mean_tensor,  # Gaussian distribution mean
                        'covariance': cov_tensor,  # Gaussian distribution covariance
                        'surface': info['surface'],
                        'objects': info['objects']
                    })
                    print(f"[DEBUG] {img_name} Gaussian distribution feature 처리 완료")
                else:
                    print(f"[WARNING] {img_name}에 대한 Gaussian distribution을 찾을 수 없음")
                    
            except Exception as e:
                print(f"[ERROR] Gaussian distribution feature 로드 중 오류: {e}")
...
class RetrievalDatabase:
    """Retrieval 데이터베이스 클래스 - Gaussian Distribution 기반 사용"""
    def __init__(self, retrieval_root="/root/datasets/UOAIS/UOAIS-Sim/retrieval", save_retrieval_results=True, output_dir="./outputs"):
        # Gaussian Distribution 기반 retrieval 사용
        self.gaussian_retrieval = GaussianDistributionRetrieval(
        ...
        print(f"Gaussian Distribution 기반 Retrieval 데이터베이스 로드 완료")
        print(f"Retrieval 결과 저장: {'활성화' if self.save_retrieval_results else '비활성화'}")
    ...
    def get_retrieval_features(self, query_image, query_bbox, top_k=5):
        """쿼리 이미지에 대한 retrieval Gaussian distribution features 반환"""
        return self.gaussian_retrieval.get_retrieval_features(query_image, query_bbox, top_k)
...
class BaselineModelRetrieval(nn.Module):
        ...
        if self.retrieval_db is not None:
            try:
                ...
                if retrieval_features:
                    print(f"[DEBUG] {len(retrieval_features)}개 UOAIS-Sim Gaussian distribution features 획득")
                    augmented_embeddings = self._apply_uoais_gaussian_features(augmented_embeddings, retrieval_features)
    ...
    def _apply_uoais_gaussian_features(self, embeddings, retrieval_features):
        """UOAIS-Sim Gaussian distribution features를 원본 embeddings에 적용"""
        ...
        for i, ret_feat in enumerate(retrieval_features):
            ...
            # Gaussian distribution features 가져오기
            mean = ret_feat['mean']  # (d,) 형태
            cov = ret_feat['covariance']  # (d, d) 형태
            similarity = ret_feat['similarity']
            
            print(f"[DEBUG] Gaussian feature - mean shape: {mean.shape}, cov shape: {cov.shape}, similarity: {similarity:.4f}")
            
            # Gaussian distribution을 이미지 임베딩에 적용하는 방법
            # 1. Mean을 spatial feature로 사용
            mean_expanded = mean.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)  # (1, d, 1, 1)
            mean_expanded = mean_expanded.expand(1, mean.size(0), height, width)
            
            # 2. Covariance의 diagonal을 variance feature로 사용
            var_diag = torch.diag(cov)  # (d,)
            var_expanded = var_diag.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)  # (1, d, 1, 1)
            var_expanded = var_expanded.expand(1, var_diag.size(0), height, width)
            
            # 유사도 기반 가중치 적용
            weight = similarity * 0.05  # Gaussian feature는 더 작은 가중치 사용
            
            # Feature 차원이 이미지 임베딩보다 작거나 같은 경우
            if mean.size(0) <= channels:
                # Mean feature 적용
                embeddings[:, :mean.size(0), :, :] += weight * mean_expanded
                
                # Variance feature 적용 (다음 채널들에)
                if mean.size(0) + var_diag.size(0) <= channels:
                    embeddings[:, mean.size(0):mean.size(0)+var_diag.size(0), :, :] += weight * var_expanded
            else:
                # Feature 차원이 큰 경우, 처음 channels만 사용
                embeddings += weight * mean_expanded[:, :channels, :, :]
            
            print(f"[DEBUG] Gaussian feature 적용 완료: weight={weight:.4f}")
