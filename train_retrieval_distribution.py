...
from baseline_model_retrieval_distribution import BaselineModelRetrieval
...
DATASET_NAME = 'pix2gestalt'  # 'd2sa' 또는 'pix2gestalt'
...
if DATASET_NAME == 'd2sa':
    ...
    OUTPUT_DIR = "./outputs/D2SA_aug_amodal_visible_retrieval_distribution_250915"
    
elif DATASET_NAME == 'pix2gestalt':
    PIX2GESTALT_ROOT = "/root/datasets/pix2gestalt_occlusions_release"
    OUTPUT_DIR = "./outputs/Pix2Gestalt_amodal_visible_retrieval_distribution_250915"
    MAX_SAMPLES = 3000
...
if __name__ == '__main__':
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # 계층적 학습 전략 적용
        if epoch <= 2:
            # 1-2 에포크: Visible 디코더만 학습 (Retrieval 비활성화)
            ...
            # 3-4 에포크: Amodal 디코더만 학습 (visible 고정, Retrieval 활성화)
            ...
            # 5-6 에포크: 전체 디코더 미세조정 (Retrieval 활성화)
        ...
        # 학습 단계에 따라 다른 메트릭 사용
        if training_stage == 'visible_only':
            current_metric = metrics.get('visible_mIoU', 0.0)
            metric_name = "visible mIoU"
        else:
            current_metric = metrics.get('amodal_mIoU', 0.0)
            metric_name = "amodal mIoU"
        
        is_best = current_metric > best_amodal_miou
        if is_best:
            best_amodal_miou = current_metric
            patience_counter = 0
            print(f"새로운 최고 {metric_name}: {current_metric:.4f}")
        else:
            patience_counter += 1
            print(f"현재 {metric_name}: {current_metric:.4f} (최고: {best_amodal_miou:.4f})")
        ...
        print(f"\n현재 최고 {metric_name}: {best_amodal_miou:.4f} (에포크 {checkpoint_manager.best_epoch})")
        
        if patience_counter >= PATIENCE:
            print(f"\n{PATIENCE} 에포크 동안 성능 개선이 없어 학습을 조기 종료합니다.")
            break

    print(f"\n=== 학습 완료 ===\n최종 최고 성능: {best_amodal_miou:.4f}")
